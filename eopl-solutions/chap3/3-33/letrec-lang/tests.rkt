...
in let times4 = (fix t4m)
 ...
 in letrec f(x) = if zero?(x) then 0 else -((f -(x,1)), m) in (f 4)"
   ...
   in (odd 13)" 1)
      ...
      (3-33-test
       "letrec fib0 (n, a, b) = if zero?(n) then b else (fib1 -(n,1) b -(a,-(0,b)))
                fib1 (n, a, b) = if zero?(n) then b else (fib2 -(n,1) b -(a,-(0,b)))
                fib2 (n, a, b) = if zero?(n) then b else (fib0 -(n,1) b -(a,-(0,b)))
         in -((fib0 10 0 1),-((fib1 7 0 1),(fib2 4 0 1)))" 73)
      ))
  )
