       ...
       (cond
         ...
         [else (extend*-env (cdr vars)
                            ...
                            (extend-env
                              ...
                              (car vals)
                              env))]))
     (let ([result (value-of body (extend*-env params vals saved-env))])
     ...
     (let ([kvs (map
       ...
       (value-of body
                 (foldr
                  ...
                  kvs)))]
    [(Proc vars body)
     (Closure vars body env #f)]
    [(TraceProc vars body)
     (Closure vars body env #t)]
    [(App proc args)
    ...
    [(Not b)
     (let ([val (value-of b env)])
       (let ([sval (val->bool val)])
         (Bool (not sval))))]
    [(LetRec pname vars pbody lbody)
     (value-of lbody (extend-env-rec pname vars pbody env))]))
