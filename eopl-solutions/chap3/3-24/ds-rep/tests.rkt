...
in let times4 = (fix t4m)
   in (times4 3)" 12)
      ...
      (times4 "let makemult = proc (maker) proc (x) proc(y)
      if zero?(x) then 0
      ...
      (odd? "let makeeven? = proc (maker)
                              proc (n)
                                if zero?(n)
                                then zero?(n)
                                else not(((maker maker) -(n,1)))
            in let odd? = proc (x) not(((makeeven? makeeven?) x))
               in (odd? 3)" #t)

      (even? "let makeeven? = proc (maker)
                              proc (n)
                                if zero?(n)
                                then zero?(n)
                                else not(((maker maker) -(n,1)))
